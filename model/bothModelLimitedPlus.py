...
class AutoNet(pl.LightningModule):
    def __init__(self, hparams):
        ...
        for i in range(6):
            if i != 1 and i != 4:
                self.reversedfc2.append(nn.Sequential(
                    nn.Linear(14 * 13 * 64, self.fc_num1 + self.fc_num2, bias=False),
                    nn.BatchNorm1d(self.fc_num1 + self.fc_num2),
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.2),
                ))
            else:
                self.reversedfc2.append(nn.Sequential(
                    nn.Linear(13 * 18 * 64, self.fc_num1 + self.fc_num2, bias=False),
                    nn.BatchNorm1d(self.fc_num1 + self.fc_num2),
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.2),
                ))
        self.reversedfc1 = nn.Sequential(
            nn.Linear(self.fc_num1 + self.fc_num2, 64 * 4 * 5, bias=False),
            nn.BatchNorm1d(64 * 4 * 5),
            nn.ReLU(inplace=True),
            nn.Dropout(0.2),
        )
        ...
        self.deconv0_for_self = self._make_deconv_layer(256, 128)
        self.inplanes = 128
        self.conv0_for_self = self._make_layer(BasicBlock, 128, 2)
        self.deconv1_for_self = self._make_deconv_layer(128, 64)
        self.inplanes = 64
        self.conv1_for_self = self._make_layer(BasicBlock, 64, 2)
        self.deconv2_for_self = self._make_deconv_layer(64, 32)
        self.inplanes = 32
        self.conv2_for_self = self._make_layer(BasicBlock, 32, 2)
        self.deconv3_for_self = self._make_deconv_layer(32, 16)
        self.conv3_for_self = nn.Conv2d(16, 3, 1)
    ...
    def forward(self, x, detection_target=None):
        ...
        x2 = self.fc1_1(x2)
        x2_for_x3 = self.limitedFC1(x2, self.fc2_1, 32)
