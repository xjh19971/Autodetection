...
class ResNet(nn.Module):
    ...
    def _make_deconv_layer(self, ratio, last=False):
        layers=[]
        if last is not True:
            layers.append( nn.ConvTranspose2d(self.num_classes * ratio *2, self.num_classes * ratio, 3, stride=2, padding=1, output_padding=1,bias=False))
...
def trainModel():
    return ResNet(Bottleneck,[3,8,6,3]);